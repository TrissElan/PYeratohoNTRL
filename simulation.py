...
class Simulation:
    ...
    def select_target(self, other: CM.Character):
        ...
        if player in MASTER_LOCATION:
            if other.target == None:
                msg = player % "는 " + other % "에게 가까이 다가간다...\n"
            elif other.target == player:
                msg = player % "는 자신에게 다가온 " + other % "에게 시선을 돌렸다.\n"
            else:
                msg = player % "는 " + other.target % "과 " + other % " 사이에 끼어들었다!\n"
    ...
    def current_info(self):
        ...
        SYSTEM.setText(
            0, f"{SYSTEM.timeInfo} / 현재위치 : {PLACE.NAME()} < {target_text} > "
        )
        ...
        for i, char in enumerate(target_list):
            ...
            tagName = f"CHAR_{i}"
            SYSTEM.DISPLAY.textArea[0].tag_add(
                tagName, f"1.{start_index}", f"1.{end_index}"
            )
            SYSTEM.DISPLAY.textArea[0].tag_bind(
                tagName, "<Enter>", lambda e, tag=tagName: SM.on_enter(e, tag)
            )
            SYSTEM.DISPLAY.textArea[0].tag_bind(
                tagName, "<Leave>", lambda e, tag=tagName: SM.on_leave(e, tag)
            )
            SYSTEM.DISPLAY.textArea[0].tag_bind(
                tagName, "<Button-1>", lambda e, target=char: self.select_target(target)
            )
