...
def showParam(CHARA, area):
    ...
    # 기본정보 설정
    BaseInfo1 = "□ " + CHARA.NAME(index = 0) + f" (호감 : {CHARA.CFLAG[20][MASTER]} | 신뢰 : {CHARA.CFLAG[21][MASTER]} | 굴복 : {CHARA.CFLAG[22][MASTER]})\n"
    BaseInfo2 = f"체력 : [{SYSTEM.fstr(CHARA.PARAM[0][0][0],4)}/{SYSTEM.fstr(CHARA.PARAM[0][0][1],4)}] | 이성 : [{SYSTEM.fstr(CHARA.PARAM[0][1][0],4)}/{SYSTEM.fstr(CHARA.PARAM[0][1][1],4)}]\n"
    ...
    if SYSTEM.GFLAG[105]:
        AddInfo2 += f"{"자지" if CHARA.TALENT[0] >= 1 else "클리"}발기 : [{BASELV(CHARA.PARAM[2][0][0])}] | 유두발기 : [{BASELV(CHARA.PARAM[2][1][0])}]\n"
    ...
    result = BaseInfo1 + BaseInfo2 + AddInfo1 + AddInfo2 + AddInfo3
